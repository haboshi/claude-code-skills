...
**v0.5 の深掘り検出（安全境界は不変）**:
- **サブドメインテイクオーバー**（`subdomain-takeover`・受動・High）: 対象＋巡回で観測した**同一登録ドメイン**の
  ホストの CNAME 鎖を dnspython で解決し、既知テイクオーバー可能サービス（GitHub Pages/S3/Heroku/Azure 等の
  内蔵テーブル）を指し、かつ応答が**未所有フィンガープリント**に一致した場合のみ提示（両方一致で FP 回避）。
  単一組織スコープ厳守（ブルートフォース列挙はしない）・DNS 照会と GET のみで非破壊。
- **既知 CVE 相関**（`js-known-cve`・受動・非egress）: 検出 JS ライブラリの版数を retire.js 形式の**内蔵オフライン
  署名DB**（`references/js-vuln-signatures.json`）と突合し、危殆版一致で具体 CVE と重大度を提示（jQuery 1.11.1
  → CVE-2020-11022/11023 等）。ライブ照会はしない。DB はスナップショット日付を持ち、**要定期更新**を注記する。
- **CSP バイパス可能性の深掘り**（`csp-bypassable`・受動）: CSP をディレクティブ解析し、`default-src` フォール
  バックを考慮のうえ明確なバイパス条件（unsafe-inline の nonce/hash 無し・script/object の広い source・
  base-uri/form-action 欠如）のみを **1 所見に集約**（`default-src 'none'` 等の実効無害な指定は指摘しない）。
- **ユーザー列挙**（`user-enumeration`・opt-in 能動・Medium）: `--active-auth` 二重ゲート内で、**非存在ランダム
  アカウントのみ**を使い login／パスワード再発行（`--reset-url`・既定 OFF）の応答がアカウント有無を露呈するかを
  観測。汎用応答なら安全側に倒し（generic override で FP 抑制）、確度は控えめ（Low）。実在アカウント不使用・改変なし。
...
| `--reset-url` | **ユーザー列挙テストの対象パスワード再発行 EP（既定 OFF・in-scope 必須）** | — |
...
- `js-vuln-signatures.json` — 既知 CVE 相関用の内蔵オフライン署名DB（retire.js 形式・snapshot 付き・非egress）
