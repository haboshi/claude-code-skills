...
`verify-structure` を最後に置くのは、生成物への絶対パス混入検査の対象に
`layout-verification.json` / `browser-verification.json` を含めるため。先に走らせると、
まだ存在しないファイルを検査しないまま通ってしまう。構造の破れ自体は `build` が止めるので、
壊れたモデルが最後まで進むことはない。
