...
- **構造の図はインライン SVG、情景・比喩・全体観の一枚絵はラスタ画像**（codex-imagegen）。ラスタ適性のブロックがあるときだけ Phase 2 末で確認を1問行う（使い分け基準と設問は §7。ユーザーが形式を指定済みなら質問しない）
...
図解は種類で振り分ける。**構造は SVG、情景はラスタ**:

- **インライン SVG（既定）**: 比較・時系列・手順・フロー・構成・分類・関係・数量 — ラベルの文字が情報の主体で、正確に読める必要がある図。実名ラベル（ファイル名・部署名・金額・件数）を含む図は必ず SVG（検索・テキスト選択・読み上げ・拡大に応えるのは SVG だけ）
- **ラスタ画像（codex-imagegen）**: 概念の比喩・利用シーン・導入前後の情景・文書全体の世界観を1枚で掴ませる絵 — 「読む」より「見て掴む」が目的の図。文字はタイトル1つ＋短いラベル数個までに抑え、詳細な文字情報は本文と SVG に持たせる
- **迷ったら**: 「図中の文字をすべて消しても骨子が伝わるか」を自問する。伝わる → ラスタ候補。文字がないと成立しない → SVG。それでも迷うなら SVG（既定）に倒す
- **枚数の目安**: ラスタは1文書に1〜3枚（表紙直後の全体観など）。構造図まで置き換えない — 白基調の業務文書でラスタが増えると様式が崩れ、容量も膨らむ（PDF 化は §10 の軽量化が前提）

ラスタ適性ブロックが1つ以上あり、かつユーザーが図解形式を指定していないときだけ、Phase 2 の outline 確定時（Phase 3 に入る前）に AskUserQuestion で1問だけ確認する。これは §2 の1ターン完走に対して認可された唯一の追加停止点で、回答を得たらそのまま完走する。該当ブロックがなければ質問せず SVG のみで進む。

```
AskUserQuestion({
  questions: [{
    question: "図解のうち「<該当ブロック名>」は、構造図（SVG）よりイラスト調の一枚絵の方が伝わると判断しました。ラスタ画像（AI 生成・白基調のビジネス画風。生成に数分かかります）を使いますか？",
    header: "図解の形式",
    options: [
      { label: "一枚絵はラスタで（推奨）", description: "情景・全体観のブロックだけ AI 生成画像にする。構造図は SVG のまま" },
      { label: "すべて SVG で", description: "ラスタを使わず全図を SVG で組む（生成待ちなし・テキスト選択可）" }
    ]
  }]
})
```

### ラスタの画風 spec（B1 ビジネスフラット — 文書内で固定）
...
```
説明画像、16:9 landscape、白背景（#ffffff。和紙・クリーム地にしない）のフラットなビジネスイラスト。
配色はアクセント色 <確定した accent 値> ＋ 濃グレー（#1f2937 系）＋ 薄グレーの3系統に制限する。
均一な太さの線と整った塗りで描く（コーポレートサイトの図版の様式。手描きマーカーの質感にしない）。
人物・情景も記号的なフラット表現にする（写実・3D・過度なグラデーション・水彩にしない）。
日本語ラベルを読みやすいサイズで正確に描く（render every character exactly and legibly）。
文字は少なめ — タイトル1つ＋短いラベル数個まで。詳細な文字情報は本文・SVG 側が担う。
矢印・番号・大小の対比で視線の流れを作り、情報の薄い装飾画にしない。
禁止: クリーム地・和紙風の背景、手描きの乱れた線、多色のごちゃつき、英語ラベル、写実的な人物、余白だらけの抽象画。
```
