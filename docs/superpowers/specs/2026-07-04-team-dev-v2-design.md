...
## 多エージェント検証の結果と適用済み修正（2026-07-04 追記）

実装完了後、6次元（設計⇔実装整合 / ループ健全性 / プラットフォーム仕様適合 / 運用実務 / ハーネス原則適合 / Codex 統合）× 敵対的検証3票の多エージェントレビュー（57エージェント）を実施した。確定指摘10件（統合後8件）はすべて修正を適用済み:

1. **[CRITICAL] TeamCreate/TeamDelete/TeamList は v2.1.178 で廃止済み**（公式 docs + 実環境 v2.1.201 で確認）→ Preflight を `~/.claude/teams/session-*` 残骸検出へ書き換え、チーム編成・シャットダウン・異常リカバリを「セッション由来名で自動生成・自動クリーンアップ」前提に全面改訂
2. **[HIGH] 「Shift+Tab で Delegate Mode」は現行仕様に不存在**（現行は permission mode 循環）→ 案内を撤去しプロンプト規律で代替
3. **[HIGH] Gate F の「統合後 diff」のベース未定義** → mission-brief.md に base-sha を記録し `git diff <base-sha>...HEAD` で生成。テスト結果はファイル保存を規定
4. **[HIGH] `codex exec review --uncommitted` が worktree 隔離・統合マージと不整合で空 diff** → 統合後は `--base <base-sha>`、実装中は worktree 内で `--uncommitted` と範囲指定を必須化（Guardian / Small 代替 / Gate F すべて）
5. **[HIGH] Medium 構成の Wave 構造・検証担当が未定義** → Medium の Wave 表を追加し、Guardian 節の代替記述を「Small / Medium（Guardian 非昇格時）」に拡張
6. **[HIGH] Claude 軸 verdict JSON の回収保証なし** → 「末尾は verdict JSON のみ・失敗時1回リトライ→escalate」の回収契約を追加
7. **[HIGH] /tmp 固定パス + `|| true` による stale verdict 誤読** → 全 Codex コマンドを「実行前 rm -f → 実行 → 存在/JSON 検証」の3ステップ化、再ゲート時は試行回数入りパス

### 敵対的検証で棄却された指摘（7件 — 再調査不要）

ラダーと revise cap の矛盾疑い / 降格中の昇格空振り疑い / 「同一指摘」判定基準の曖昧さ / Small の前段読み替え未規定 / verifier への元要件欠落 / 元要件正本の不在 / 縮退時の二軸一致空集合化 — いずれも3票検証で原文照合・仕様裏取りにより反証済み。

### 既知の残課題（未検証 MEDIUM/LOW 30件 — 次サイクルで選別）

- **仕様整合系**: Agent tool のモデル解決順に `CLAUDE_CODE_SUBAGENT_MODEL` が欠落 / チームメイトのモデル解決順の後半2つの優先関係が未裏取り / fallbackModel はレート制限では発動せず切替もターン限り（保険としての位置づけが過大）/ フックサンプルの応答 JSON が公式スキーマと衝突（SubagentStop の continue の意味逆転）/ TeammateIdle サンプルが stdin を読んでいない / 「permission 個別設定不可」は spawn 後は変更可能
- **ループ健全性系**: ラダーのカウンタが Gate 2→3 と Gate F で共有か独立か未定義 / エラーリカバリー Level 2 に回数上限なし / カバレッジ・テスト失敗はラダー適用外で cap が効かない / evaluator 発 escalate の処理未定義 / セルフレポート再 spawn に回数上限なし / 計画欠陥の差し戻し先が実装者ラダーしかない
- **ゲート堅牢化系**: 二軸の status 不一致の解決規則未定義 / ゲート状態（revise 回数・前回 defects）が compaction で消える / Gate P に決定論前段なし / verdict 語彙（fail）と契約 enum の不整合 / Gate F が Maker 自己申告のテスト結果を鵜呑み / Codex の `--output-schema` 未活用 / Gate P のシェル引数展開のクォート崩れリスク / Codex 軸プロンプトへの verdict スキーマ展開指示欠落 / verifier Task のモデル明示なし
- **運用系**: 稼働モデル自己報告のオウム返し偽陽性 / Guardian レビューの頻度・コスト未定義 / build/tsc ハードコードで非 Node プロジェクト不可 / Small の実装者1名の定義・隔離未規定 / 報告項目に worktree パス欠落 / Codex のモデル非固定規律のゲート節への未転記 / Tester の Codex 検証が低シグナル
