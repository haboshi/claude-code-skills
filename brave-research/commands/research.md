...
依頼文と直前の会話から次を自分で決める（brave-research スキル本文の「調査開始前の判定」と同じ判定表を使う）:

- **調査の目的**: 技術選定 / 競合分析 / トレンド把握 / 学習 / その他（依頼文の動詞と対象から読む）
- **深度**: 概要レベル / 詳細分析 / 網羅的調査（既定: 詳細分析。「ざっと」→ 概要、「網羅的に」「漏れなく」→ 網羅）
- **出力形式**: 会話内テキスト / Markdown ファイル保存（既定: 会話内。「ファイルに」「保存して」→ ファイル）

読み違えると調査の中身が別物になり、かつ会話から決められない 1 点だけ AskUserQuestion で 1 問（推奨を先頭）。
それ以外は既定で進め、レポート冒頭に採った前提を 1 行書く。
...
計画は提示するだけで、承認を待たずに Step 2 へ進む（調査は依頼済みの作業。方向を変えたければユーザーが途中で口を挟める）。
...
全情報を構造化レポートにまとめる。各ソースの内容は自分の言葉（間接話法）で 1〜2 文に言い換え、原文のまま残すのは
引用符付きの短い語句を 1 つまでとして出典を添える。形はスキル本文（brave-research SKILL.md「出典の言葉の扱い」）の
`<example>` と同じ: 一致点と相違点を軸に構成し、どの記事の要約にもならないようにする。
