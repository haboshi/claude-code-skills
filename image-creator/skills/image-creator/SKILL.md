...
## Step 0: ユースケース判定（最初に実行・質問は最小限）

技術詳細（プロバイダー等）より先に「何を作りたいか」を下の 5 タイプに当てはめる。依頼文・添付・直前の会話で
タイプが一意に定まるなら AskUserQuestion を使わず、そのまま次のステップへ進む（例: 「YouTube のサムネ作って」→
リッチ画像、「透過のアイコンを 3 つ」→ シンプル画像（透過）、「この手順を図解して」→ Step 0.5）。

### 画像タイプ5択

| タイプ | 会話からの手がかり | 次のステップ |
|---|---|---|
| YouTubeサムネイル・ブログヘッダー | サムネ・ヘッダー・アイキャッチ・OGP | → Step 1: リッチ画像（`generate_rich.py`） |
| 説明図・図解・比較図 | 図解・図にして・可視化・比較・フロー | → Step 0.5: 図種サジェスト |
| アイコン・ステッカー（透過背景） | アイコン・ステッカー・透過・ロゴ | → Step 1: シンプル画像（透過） |
| 一般的なイラスト・写真 | イラスト・写真・絵・風景・キャラ | → Step 1: シンプル画像 |
| テキスト入り画像（文字精度重視） | 文字を入れて・見出し入り・日本語の文字 | → Step 1: GLM-Image |

複数タイプに読めて、どれを取るかで生成物が別物になるときだけ AskUserQuestion で 1 問（上の 5 択、推奨を先頭）。
迷う程度なら既定（一般的なイラスト・写真＝無課金の codex 経路）で 1 枚作り、外れていれば作り直す方が速い。
残り 4 タイプは従量課金の経路（Gemini / OpenAI / GLM）なので、そちらへ倒すか迷うときは先に無課金経路で試す。

### Step 0.5: 図種サジェストテーブル

「説明図・図解・比較図」に判定した場合（「〇〇を図にして」「〇〇を可視化して」等）、以下から最も近い行を 1 つ選び、推奨図種の先頭で進む。伝えたい内容が表の複数の行に当てはまり、図種で成果物が別物になるときだけ、推奨図種 top3 を AskUserQuestion で 1 問提示する（推奨を先頭に置く）。
...
### 追加で決める項目（会話から読み取り、読めなければ既定）

- **サムネイル**: スタイル（アニメ/ビジネス/写真。既定: ビジネス）、アニメ風なら雰囲気（驚き/インパクト/ポップ/明るい。既定: 明るい）、キャラクター有無（既定: なし）
- **説明図**: タイプ（図解=graphrec / 比較=comparison / プロセス=process / カスタム。既定: 内容に最も近いもの、迷えば graphrec）

AskUserQuestion は、読み違えると生成物が別物になる 1 点だけに使う（例: アニメ風か写真風かで参照画像の扱いが変わるとき。推奨を先頭に置く）。それ以外は既定で 1 枚作り、結果を見てから直す。
