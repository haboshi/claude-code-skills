...
- **VAD 設定は `VadConfig`（`semantic_vad` / `server_vad` / `local` の3方式）を受け取り、プロバイダの
  wire フォーマットへアダプタ内で変換する**（方式の選択は `references/session-lifecycle.md`「VAD 3方式
  の選び方」、パラメータ値の決定は `references/tuning-guide.md` を参照）
...
| `references/tuning-guide.md` | VAD（eagerness / threshold / silence 等）・reasoning effort・noise reduction の値を利用シーンに合わせて決めるとき |
