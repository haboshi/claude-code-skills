...
# Deep Research v4.4 - マルチエージェント調査スキル
...
**注入検証**: エージェントプロンプト構築後、空値セクションが含まれていないことを確認する。
...
バックグラウンドエージェントは完了時に自動通知される。全通知を受信後:

1. 各出力ファイルの存在を Read で確認
2. ファイルが存在しない場合 → タスク通知テキストからレポート本文を抽出し、Write で出力ファイルパスに保存（リカバリ）
3. 全ファイル確保後、Phase 2.5 に進む

### Phase 2.5: GAP ANALYSIS（条件付き）

全バックグラウンドエージェントの完了後、最終統合の前にギャップ分析を行う。

#### Step 2.5.1: 出力ファイルの読み込みと評価

Step 2.3 で確保済みの各エージェント出力を Read で読み込み、以下を評価する:
...
- **不確実性マーカーの保持** — エージェントが「未確認」「推計」「基準年不明」等と付記した情報は、統合レポートでもその不確実性注記を保持する
...
#### Step 3.3: 引用検証（サンプリング）
- 統合レポート内のURLから **3〜5件をランダム抽出** し、WebSearch で実在を確認（全件検証は不要）
- 架空URLが見つかった場合は除去し、同一エージェントの他URLも追加検証
- 検証できなかった引用は「未検証」と明記
...
**WebSearch 回数目安**: 全ラウンド合計で **8〜15回**。1ラウンドあたり3〜4回を基準とする。
...
## 出力ルール
- **Write ツール**を使用して {出力ファイルパス} に保存する（Bash ではなく Write を使うこと）
- 言語: 日本語（英語ソースも日本語で要約）
- 各知見に出典URLを付記
- 推測や未確認情報は「未確認」と明記
- 情報の日付を記載し、古い情報（2年以上前）は明示
- レポートの目安: **100〜200行**（過不足ない情報密度を維持）
- **フォールバック**: Write でファイル保存できない場合、レポート全文をそのまま返答に含めること
```
