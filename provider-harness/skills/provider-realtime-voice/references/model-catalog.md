...
レイテンシ許容度・失敗コストに応じて上げ下げ」を推奨。**既定値は一次情報同士が矛盾**: OpenAPI spec
（v2.3.0）は `RealtimeReasoningEffort` に `default: low` を明記する一方、移行ガイド原文は「Set
reasoning effort to `low` instead of the default」と low が既定でない前提のまま（2026-07-12 検証。
「未確認事項」参照）。本番は既定に依存せず必ず明示指定する。本スキルのポートでは
...
- OpenAI `reasoning.effort` の**実効既定値**（2026-07-12 更新: 一次情報同士が矛盾している。OpenAPI
  spec v2.3.0 は `default: low` を明記、移行ガイドは「Set reasoning effort to `low` instead of the
  default」と low 非既定の前提のまま。spec と guide のどちらが実挙動かは Pin+Verify で確認する。
  本番は明示指定が必須である点は両ソースで共通）
