...
9. **「14px 相当」は実表示サイズ基準**: viewBox 幅が本文幅（約 780px）より広い図は `width:100%` で縮小表示される。
   実表示の文字サイズ ≈ font-size × 780 ÷ viewBox幅 が 14px を下回らないよう、viewBox を広げたら font-size も上げる
   （実例: viewBox 幅 1140 で font-size 15 のままだと実表示 約10px になり違反。22 以上に上げる）
