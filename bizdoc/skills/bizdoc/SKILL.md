...
- **見出しは「役割ラベル — 主張句」の二部構成にする（v0.12.0）**: 各セクションの `h2` は `<役割ラベル> — <用言を含む主張句>` の 1 形式で全節を統一する（例: 「品質担保 — 目視ではなく 2 つの決定論ゲートが守る」「本番操作 — push 1 回だけ、触るのは main の一時 worktree」）。役割ラベルは節の種類を 2〜5 字で告げる語（現状／前提／手順／本番操作／完了判定／裏取り／設計判断／提案／代替案／落とし穴／残リスク／効果／判断依頼／切戻し／付録 など。文書ごとに選び、固定の章順にはしない）。主張句は読者への主張を用言で言い切る（20 字以内目安）。役割ラベルだけの見出し（「背景」「まとめ」）も、役割ラベルを欠いた主張文だけの見出しも作らない
    - **なぜこの形か**: 2026-09-08 の同一文書 A/B（原本＝主張文のみで断定・疑問・機能ラベルの 3 型混在 ／ 案A＝役割ラベル＋短い句、主張は `.sec-lede` ／ 案B＝役割ラベル — 主張句）で、案B が明確に「頭に入る」と評価された。役割ラベルが「どの種類の節か」を、主張句が「何を言うか」を同じ行で同時に渡すので 1 チャンクで届く。案A は h2 と lede の 2 行を読んで合成しないと節の意味が確定せず、h2 の体言止めの断片が命題にならない。原本は種類の手がかりが無く、節ごとに見出しの形式が変わって読み方を切り替えさせていた。よって v0.9.0 の「構文を散らす」「疑問形の許可枠」「機能ラベルの条件付き許可」は廃止する — 単調さの対策は形式を混ぜることではなく、形式を固定して主張句の中身（対比・数値・否定・条件）で変化を出す
    - **疑問は答えの形で書く**: 「なぜ反映しただけでは足りないのか」と立てたい節は「前提 — 反映だけでは足りず、同意データが要る」のように主張句で答えを言う。一覧・カタログの節も「付録 — 予行演習のタイムライン（約 6 分）」「内訳 — 1 本あたりの費用は 3 要素で決まる」のように役割ラベル＋句の形を崩さない
    - **1見出し1主張**: ダッシュ（—）は役割ラベルと主張句の区切り専用。主張句に独立した 2 つ目の主張を足さない（`.sec-lede` か本文へ落とす、または節を分ける）
    - **用言で言い切る（主張句）**: 体言止め・名詞化（「〜の拡大」「〜済み」「〜の活用開始」）をやめ、動詞・形容詞で終える（「〜が増えた」「〜を確認する」「〜だけ」「〜が要る」）。断定の常体で書けば口語にはならない。役割ラベル側は名詞でよい
    - **未解決要素を1つ残す**: 主張句は本文が回収する未解決要素（なぜ／どうやって／何を捨てたか）を1つ残す。目次を上から読んだとき全見出しが「答えの言い切り」で完結している outline は差し戻す（見出しを読んだ時点で本文を読む理由が消える）
    - **骨格可読性**: `h2` の列だけ（`.sec-lede` を読まずに）拾い読みして文書の論旨が再構成でき、役割ラベルの列だけで文書の骨格（現状→手順→判定→切戻し 等）が辿れること（Phase 5 の検査項目）
    - **漢語のアドホック連結禁止**: 漢字5字以上の連結語は、一語として定着した語（「費用対効果」等）・固有名詞・数値単位を除き見出しに置かない。「分析対象範囲拡大」型の即席連結は動詞へ開く（「分析する範囲が広がった」）
    - **長さ上限**: 役割ラベル 5 字以内、主張句 20 字以内を目安、見出し全体で全角 35 字以内。超えるときは語を削って圧縮するのではなく、主張を1つに絞る
    - 改稿例: 「費用増の原因は特定済み — モデルの問題ではなく、分析対象の拡大」→「原因 — 費用が増えたのはモデルではなく分析対象が増えたから」／「なぜ反映しただけでは足りず、同意データを入れるのか」→「前提 — 反映だけでは足りず、同意データが要る」／「済んでいること・残っていること」→「現状 — 準備は済み、残るのは外部入力 1 つ」
- **見出しのレジスター（文体水準）を縛る**: 「主張句」は主張の強さの規範であり、圧縮の免罪符ではない。A/B 実測で bizdoc 見出しの認知負荷が高いと評された残存要因は、格式を漢語の圧縮で出そうとする癖にある — 格式は内容の確かさで出す。対象はセクション見出し（`h2`・outline の主張句）で、§8 の目次項目も同じ形式（役割ラベル — 短縮した主張句）で書く
...
- **`.sec-lede` は半歩先のスポイラー**: 見出しの主張の「意外な一点」（何が常識と違うか・何を変えないか）を1行で書く。**1 行＝全角 60 字以内**（`.sec-lede` は h2 と同じ器幅 954px・15px で組まれ、約 63 字で折り返す。超えると 2 行目に数文字の端切れが出て読みにくい — 2026-09-08 実指摘。収まらなければ後半を本文の第 1 段落へ落とす）。手続き文（「〜を要約します」）・免責・換算前提はここに置かない（本文末尾か出典行へ）。決裁レイヤー on のときは二重契約 — 逐次読者への半歩先スポイラーであると同時に、途中参入する決裁者が「この節の問いと答え」を局所復元できる形にする
...
- **予防則 2・3・4・5・9・10 は `hub.mjs add` が機械判定して warn する**（v0.12.1）。ただし判定できるのはここまでで、**11〜15（無名の箱・読み順と終端・実データ標本・反復の個数・強調は同時1点）は題材を知らないと真偽が決まらない**ため人が見る。機械判定があることは、README.md を読まずに書いてよい理由にはならない
...
- **セクション番号・図番号の機構（v0.11.1 / v0.11.2）**: 番号は tokens.css の CSS カウンタ（`section > h2 { counter-increment: sec }`）で付く。
  ...
  目次項目は `h2` と同じ「役割ラベル — 主張句」の形のまま、主張句だけを短縮して **合計 25 字程度**にする（役割ラベルは省略しない — 目次は役割の列だけで骨格が辿れる地図にする。h2 全文はそのままでは 2 カラムに収まらない）。
...
<nav class="toc">
  <h2>目次</h2>
  <ol>
    <li><a href="#s-01">現状 — 1 通話に最大 9 回の解析</a></li>
    <li><a href="#s-02">原因 — 費用増はモデルでなく分析対象の拡大</a></li>
  </ol>
</nav>
```
...
- 続けて `add` は骨格（目次リンクと h2 の対応）と**図の予防則（2・3・4・5・9・10）を機械判定し、stderr に warn を出す**（v0.12.1）。**warn を1件でも残したまま完了しない** — 出たら直して `--update` で再保存する。判定できない予防則11〜15 は下の crop 目視で見る
...
- **骨格検査**: `h2` の列だけを拾い読みし、(a) 論旨が再構成できるか、(b) 役割ラベルの列だけで骨格が辿れるか、(c) 全見出しが「答えの言い切り」で完結して読む理由が消えていないか（未解決要素が残っているか）を確認する
- **見出しの形式検査（v0.12.0）**: 全 `h2` が「役割ラベル — 主張句」の 1 形式か（役割ラベルだけ・主張文だけ・疑問文だけの見出しが混ざっていないか）、役割ラベルが節の中身と合っているか（手順を並べる節に「現状」等）を数える。混ざっていれば §6 の見出し規範に戻る
- **`.sec-lede` が 1 行に収まっているか** — 2 行目に数文字の端切れが出ていれば 60 字超。文を短くするか、後半を本文の第 1 段落へ落とす
...
- **図の予防則は「読む」約束だけに頼らない** — v0.12.0 までは「埋め込む前に必ず `svg-patterns/README.md` を Read する」という人手の約束しか無く、守られたかを確かめる手段が無かった。実際、読まずに書いた図が予防則3（`<title>` 欠落）・4（font-family 指定）・5（色リテラル13種）・9（font-size 13）を同時に外したまま保存された（2026-09-09）。v0.12.1 から `add` が機械判定するが、**判定できるのは6項目だけ**で、残り（無名の箱・読み順と終端・強調は同時1点）は人が見るしかない
...
- **見出しが片翼になっていないか仕上げ段階でも見る** — Phase 5 の PNG 確認時に見出しだけを拾い読みし、役割ラベルだけ（「背景」）や主張文だけの見出しが混ざっていれば Phase 2 の見出し規範（役割ラベル — 主張句）に戻って書き直す
