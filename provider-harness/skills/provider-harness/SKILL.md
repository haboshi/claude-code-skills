...
| リアルタイム音声 | provider-realtime-voice | 実装済み |
...
予約領域は本メタ原則のみで実装し、完了後 harvest で型化候補として報告する（ドメインスキルへの昇格判断は人間が行う）。昇格の判断基準・6段階のパイプライン（発見スキャン→実コード収穫→外部一次情報調査→執筆→ブラインドA/B審査→登録）は `references/domain-genesis.md` を参照。
...
| `references/domain-genesis.md` | 予約領域をドメインスキルへ昇格させるか判断する時 |
