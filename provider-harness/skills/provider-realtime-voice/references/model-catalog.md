...
レイテンシ許容度・失敗コストに応じて上げ下げ」を推奨。**既定値は一次情報で明示されておらず未確認**
（「未確認事項」参照）。本スキルのポートでは `RealtimeSessionConfig.reasoningEffort` として正規化
してある（`templates/port.ts` 参照）。
...
- OpenAI `reasoning.effort` の**既定値**。プロンプティングガイドの gpt-realtime-2 移行手順が
  「既定のままにせず `low` を明示指定せよ」と書いており、既定 = low とは限らない（二次報道には
  low 既定とするものがあるが一次情報で未確認）。いずれにせよ本番は明示指定が公式推奨
