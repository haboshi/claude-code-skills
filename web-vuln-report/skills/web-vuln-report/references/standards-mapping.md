...
本表は全 42 チェック（`check_id`）を、**CWE・OWASP Top 10:2025・OWASP WSTG v4.2・
...
| `route-disclosure` | SPA/JS ルート・エンドポイント名の未認証露出 | CWE-200 | A01:2025-アクセス制御の不備 | WSTG-INFO-05 | 該当なし | `AV:N/AC:L/AT:P/PR:N/UI:N/VC:L/VI:N/VA:N/SC:N/SI:N/SA:N` | 6.3（Medium） |
| `unauth-sensitive-route` | 機微パスが未認証 GET で 200 応答（保護確認の疑い） | CWE-425 | A01:2025-アクセス制御の不備 | 該当なし | 該当なし | `AV:N/AC:L/AT:P/PR:N/UI:N/VC:L/VI:N/VA:N/SC:N/SI:N/SA:N` | 6.3（Medium） |
| `csrf-not-enforced` | anti-CSRF トークン無しの POST が受理される（能動・opt-in） | CWE-352 | A01:2025-アクセス制御の不備 | WSTG-SESS-05 | v5.0.0-3.5.1 (L1) | `AV:N/AC:L/AT:P/PR:N/UI:A/VC:N/VI:L/VA:N/SC:N/SI:N/SA:N` | 2.1（Low） |
| `stack-fingerprint` | フレームワーク/インフラスタックの指紋（情報） | CWE-200 | A01:2025-アクセス制御の不備 | WSTG-INFO-08 | 該当なし | `AV:N/AC:L/AT:N/PR:N/UI:N/VC:N/VI:N/VA:N/SC:N/SI:N/SA:N` | 0.0（Info） |
...
| `js-secret-exposure` | JS バンドル内の秘密情報の露出 | CWE-540 | A02:2025-セキュリティの設定ミス | WSTG-INFO-05 | 該当なし | `AV:N/AC:L/AT:N/PR:N/UI:N/VC:H/VI:N/VA:N/SC:N/SI:N/SA:N` | 8.7（High） |
| `subdomain-takeover` | サブドメインテイクオーバーの疑い（dangling CNAME） | CWE-668 | A02:2025-セキュリティの設定ミス | WSTG-CONF-10 | 該当なし | `AV:N/AC:L/AT:N/PR:N/UI:N/VC:H/VI:N/VA:N/SC:N/SI:N/SA:N` | 8.7（High） |
| `sourcemap-exposure` | ソースマップ（.map）の公開 | CWE-540 | A02:2025-セキュリティの設定ミス | WSTG-INFO-05 | 該当なし | `AV:N/AC:L/AT:P/PR:N/UI:P/VC:L/VI:N/VA:N/SC:N/SI:N/SA:N` | 2.3（Low） |
| `dnssec-missing` | DNSSEC 未署名（DNS 応答の完全性が検証されない） | CWE-345 | A02:2025-セキュリティの設定ミス | 該当なし | 該当なし | `AV:N/AC:L/AT:P/PR:N/UI:P/VC:N/VI:L/VA:N/SC:N/SI:N/SA:N` | 2.3（Low） |
...
| `js-known-cve` | 既知のCVEを含むJSライブラリ（署名DB照合） | CWE-1395 | A03:2025-ソフトウェアサプライチェーンの障害 | 該当なし（独自根拠） | v5.0.0-15.2.1 (L1) | `AV:N/AC:L/AT:N/PR:N/UI:A/VC:N/VI:N/VA:N/SC:L/SI:L/SA:N` | 5.1（Medium・DB severity で per-finding 上書き） |
...
| `eol-runtime` | サーバランタイム/コンポーネントの upstream EOL 疑い | CWE-1104 | A03:2025-ソフトウェアサプライチェーンの障害 | WSTG-INFO-02 | v5.0.0-15.2.1 (L1) | `AV:N/AC:L/AT:P/PR:N/UI:A/VC:L/VI:L/VA:N/SC:N/SI:N/SA:N` | 2.1（Low） |
...
| `tls-cert-invalid` | TLS 証明書の検証失敗（ホスト名不一致/期限切れ/自己署名/チェーン不備） | CWE-295 | A04:2025-暗号化の失敗 | WSTG-CRYP-01 | v5.0.0-12.1.1 (L1) | `AV:N/AC:L/AT:P/PR:N/UI:P/VC:H/VI:N/VA:N/SC:N/SI:N/SA:N` | 6.0（Medium） |
...
| `csp-bypassable` | CSP にバイパス可能な設定（object-src/base-uri/form-action/script-src） | CWE-693 | A06:2025-安全でない設計 | 該当なし（独自根拠） | v5.0.0-3.4.3 (L2) | `AV:N/AC:L/AT:P/PR:N/UI:A/VC:L/VI:L/VA:N/SC:N/SI:N/SA:N` | 2.1（Low） |
...
| `no-rate-limit` | ログイン試行のレート制限が観測されない（能動・opt-in） | CWE-307 | A06:2025-安全でない設計 | 該当なし | v5.0.0-2.2.1 (L1) | `AV:N/AC:L/AT:P/PR:N/UI:N/VC:L/VI:L/VA:N/SC:N/SI:N/SA:N` | 6.3（Medium） |
| `user-enumeration` | ユーザー列挙の可能性（アカウント有無の露呈・能動・opt-in） | CWE-204 | A06:2025-安全でない設計 | WSTG-IDNT-04 | 該当なし | `AV:N/AC:L/AT:P/PR:N/UI:N/VC:L/VI:N/VA:N/SC:N/SI:N/SA:N` | 6.3（Medium） |
| `missing-coop` | Cross-Origin-Opener-Policy (COOP) 未設定 | CWE-693 | A06:2025-安全でない設計 | 該当なし | 該当なし | `AV:N/AC:L/AT:P/PR:N/UI:A/VC:L/VI:N/VA:N/SC:N/SI:N/SA:N` | 2.1（Low） |
| `xss-protection-legacy` | 非推奨の X-XSS-Protection が有効値で設定 | CWE-16 | A06:2025-安全でない設計 | 該当なし | 該当なし | `AV:N/AC:L/AT:P/PR:N/UI:A/VC:N/VI:N/VA:N/SC:N/SI:N/SA:N` | 0.0（Info） |
...
計 47 チェック（OWASP 内訳: A01=11 / A02=13 / A03=4 / A04=6 / A05=1 / A06=10 / A08=1 / A10=1）。
v0.4 で 11 チェックを追加（A01: route-disclosure / stack-fingerprint / unauth-sensitive-route /
csrf-not-enforced、A02: js-secret-exposure / sourcemap-exposure / dnssec-missing、A03: eol-runtime、
A06: missing-coop / xss-protection-legacy / no-rate-limit）。v0.4.1 で `tls-cert-invalid`（A04）を追加。
v0.5 で 4 チェックを追加（A02: subdomain-takeover〈受動・High〉、A03: js-known-cve〈受動・非egress〉、
A06: csp-bypassable〈受動〉/ user-enumeration〈opt-in 能動・Medium〉）。`js-known-cve` の CVSS-B 5.1 は
既定ベクタで、実 finding は DB severity に応じ per-finding で low/medium/high/critical のベクタに上書きする。
...
- **v0.4 の追加（単一アプリ深掘り）**: 受動で `route-disclosure`（Ziggy/Inertia/Next のルート
  露出・disclosure 止まりで 6.3）/ `stack-fingerprint`（情報 0.0）/ `eol-runtime`（内蔵オフライン
  EOL 表・バックポート注記・断定回避）/ `dnssec-missing`、上限付き能動で `js-secret-exposure`
  （高特異度パターン・公開鍵 pk_live/AIza は誤検知せず・生値非掲載で 8.7）/ `unauth-sensitive-route`
  / `sourcemap-exposure`（version:3 実体確認で HTML フォールバック除去）、ヘッダ補完で `missing-coop`
  / `xss-protection-legacy`。**opt-in（既定 OFF）能動認証**で `no-rate-limit` / `csrf-not-enforced`
  を追加。能動認証は `_SafeClient` に触れず POST 限定・login URL 限定の `_ActiveAuthClient` で隔離し、
  実在アカウントを使わず試行を上限（8）にクランプする。真のログインレート制限/ユーザー列挙/CSRF-POST
  検証の完全自動化は非破壊の範囲外であり、必要時は認可付き手動ペンテストを推奨する（誠実性の明記）。
  TLS 暗号スイート列挙は client 側 OpenSSL が旧スイートを交渉できず偽陰性を生むため内蔵せず、
  `external_tools.py` の testssl.sh ブリッジに委譲する。
- **v0.5 の追加（単一組織スコープの深掘り・安全境界は不変）**: 受動で `subdomain-takeover`
  （対象＋観測済み同一登録ドメインのホストの CNAME 鎖を解決し、既知テイクオーバー可能サービス内蔵
  テーブルを指し**かつ**未所有フィンガープリント応答に一致したときのみ提示・両方一致で FP 回避・
  DNS 照会と GET のみ・8.7 High）/ `js-known-cve`（検出 JS 版数を retire.js 形式の内蔵オフライン署名DB
  `js-vuln-signatures.json`〈snapshot 2026-07〉と突合し具体 CVE を提示・**ライブ照会しない＝非egress**・
  DB severity で per-finding にベクタ/スコアを原子的に上書き・要定期更新を注記）/ `csp-bypassable`
  （`default-src` フォールバックを考慮した CSP 解析で明確なバイパス条件のみ 1 所見に集約）を追加。
  **opt-in（既定 OFF）能動**で `user-enumeration`（非存在ランダムアカウントのみで login/reset の存在
  露呈を観測・generic override で FP 抑制・確度 Low・csrf/rate-limit と cap を食い合わない独立
  `_ActiveAuthClient`・到達不能は `inconclusive` を明示 record）を追加。`--reset-url` は既定 OFF・
  in-scope 必須。ユーザー列挙の完全自動化は非破壊の範囲外であり、確度の高い評価は認可付き手動
  ペンテストを推奨する（誠実性の明記）。
