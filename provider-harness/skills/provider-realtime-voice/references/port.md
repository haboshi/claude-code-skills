...
Gemini の `serverContent.interrupted` は「ユーザ発話検出により進行中の応答が中断された」ことの通知で
あり、意味的には OpenAI の `input_audio_buffer.speech_started`（ユーザ発話の検出契機）と同じ役割を
果たす。新しい `RealtimeEvent` 種を追加せず既存の `speech.started` に統合したのはこのため。専用の
`response.interrupted` 相当のイベントを別途持つ案も検討したが、`speech.started` だけで「ユーザ発話に
よる中断が起きた」ことをアプリ側は判別でき、union を増やす便益が薄いと判断した。

## barge-in capability の二軸分離（serverAuto / clientCancel）

`RealtimeCapabilities.bargeIn` は当初 `boolean` 単一フィールドだったが、これは「サーバが自動で
barge-in するか」と「アプリ側が能動的に応答をキャンセルできるか」という**別の能力**を1つの値に
押し込めてしまい、Gemini のレビューで誤符号化を招いた（`serverContent.interrupted` フラグの存在＝
自動 barge-in に対応しているにもかかわらず `bargeIn: false` としていた）。

`bargeIn: { serverAuto: boolean; clientCancel: boolean }` に分離し、各プロバイダの実際の対応状況を
正直に表現する。

| プロバイダ | serverAuto | clientCancel | 根拠 |
|:---|:---|:---|:---|
| OpenAI | true | true | VAD 有効時にサーバ側が自動で応答をキャンセルし、`response.cancel` によるアプリ側能動キャンセルにも対応する |
| Gemini | true | false | `serverContent.interrupted` フラグはサーバが自動的に応答を打ち切ったことの通知（一次情報で確認済み。`model-catalog.md`「VAD」節）。一方、アプリ側から能動的に応答をキャンセルする公式な手段は一次情報で確認できていない（`model-catalog.md`「未確認事項」参照） |

Gemini アダプタの `interrupt()` は、従来 `realtimeInput.activityEnd` を送出して疑似的なキャンセルを
試みていたが、`activityEnd` は手動 VAD 構成における「入力ターンの終了」を伝える通知であり、進行中の
応答をキャンセルする意味は持たない（意味論的な誤り）。現在は `clientCancel: false` を正直に反映し、
`interrupt()` 呼び出し自体を `RealtimeVoiceError({ kind: 'unsupported' })` で拒否する実装にした。
自動 barge-in（`serverAuto`）は引き続き `serverContent.interrupted` → `speech.started` の経路で
観測できるため、アプリ側が barge-in を検知する手段自体は失われていない。
...
## H4: OpenAI interrupt() のコンテキスト整合（conversation.item.truncate 未送信）

従来の OpenAI `interrupt()` は `response.cancel` + `input_audio_buffer.clear` のみを送出していた。
これは進行中の応答生成を止め、未送信の入力バッファをクリアする点では正しいが、**サーバ側の会話
コンテキスト**（`conversation.item`）には、実際にはユーザに聴取されなかった音声全体がそのまま
残り続ける。ユーザが応答の途中で割り込んだ場合、会話履歴上は「最後まで再生された」ことになって
いるが、実際に聴取されたのは途中までであり、両者が乖離する。

OpenAI Realtime API は `conversation.item.truncate`（`item_id` / `content_index` / `audio_end_ms`）で
アイテムの音声コンテンツを実際の再生済み位置で切り詰められる。`interrupt(opts?: { itemId?: string;
audioEndMs?: number })` として省略可能引数を追加し、両方指定時のみ `conversation.item.truncate` を
追加送出するようにした。呼び出し側が実際の再生位置（`audioEndMs`）を把握していない場合に
`audio_end_ms: 0` 等の不正確な値で切り詰めてしまう事故を避けるため、`itemId` と `audioEndMs` は
どちらか一方だけの指定では送出しない（両方揃って初めて意味のある切り詰めができるため）。
`content_index` は本テンプレートが単一音声コンテンツのみを扱う前提のため `0` に固定してある
（複数コンテンツを持つカスタムアイテムを使うアプリでは、呼び出し側で拡張が必要）。

再生位置を追跡していない呼び出し側は `opts` を省略でき、その場合は従来どおり `response.cancel` +
`input_audio_buffer.clear` のみが送出される（後方互換）。
...
|:---|:---|:---|:---|
| `bargeIn.serverAuto` | true | true | OpenAI は VAD 有効時にサーバ側が自動で応答をキャンセルする設計。Gemini は `serverContent.interrupted` フラグで自動打ち切りを通知する（詳細は「barge-in capability の二軸分離」節参照） |
| `bargeIn.clientCancel` | true | false | OpenAI は `response.cancel` によるアプリ側能動キャンセルに対応。Gemini は公式な能動キャンセル手段が一次情報で確認できていない |
