...
併記する。出典が「社内試算」のような概算値であっても省略しない。

**置き場所は SKILL.md §8 が正**（v0.9.0 で統一）: 出典行は**セクションにつき原則1本、節末に集約**する。
同一出典を要素ごとに繰り返さない（KPI カード内に個別出典行を置かず、グリッド直後に1本で束ねる）。
1つの節に**出所の異なる数値**が混在するときだけ、その数値ブロックの直後に個別の出典行を置く
（「後で数値だけが切り離されて独り歩きするのを防ぐ」目的は、出所が1つなら節末1本で達成できる）。
