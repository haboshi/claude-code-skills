<!-- last_verified: 2026-07-12 / stale_days: 45 -->
...
|:---|:---|:---|:---|
| `low` | ユーザーにゆっくり話させる（大きめチャンク） | 8秒【公式】 | ディクテーション・長い読み上げ・途中で切られたくない場面 |
| `medium` | 中間 | 4秒【公式】 | 電話・コールセンターの初期値に推す声が強い【実測】 |
| `high` | できるだけ早くチャンク化する | 2秒【公式】 | 短コマンド・速い応答が欲しい対話・transcription を速く返したい場面 |
| `auto`（**既定**） | `medium` と等価【公式】 | 4秒【公式】 | まず auto で開始し、誤割り込み/遅さを見て動かす |

最大待機の具体値は OpenAPI spec の eagerness 定義に公式明記されている（「`low`, `medium`, and `high`
have max timeouts of 8s, 4s, and 2s respectively」。2026-07-12 確認）。transcription モードでも
eagerness はチャンク分割に影響する【公式】。
...
- 両方 `false` にすると「モデルは自動応答しないが VAD イベントは発火し続ける」＝発話区間の検出だけ
  ...
  複数話者の交通整理に有用
- **既定はともに `true`**（OpenAPI spec の `default: true`。2026-07-12 確認）。つまり VAD 有効の
  会話モードでは「自動応答＋自動 barge-in」が既定挙動で、切りたい側だけ `false` を明示する
...
- **既定値は一次情報同士が矛盾している**（2026-07-12 確認）: OpenAPI spec は `RealtimeReasoningEffort`
  に `default: low` を明記する一方、公式移行ガイドは「Set reasoning effort to `low` **instead of the
  default**」と low が既定でない前提の記述のまま。どちらが実挙動かは実疎通（Pin+Verify）で確認する
  こと。いずれにせよ**本番では必ず明示指定する**（既定に依存しない）のが両ソース共通の安全策
...
4. 電話経路の threshold 0.6–0.7 のような【実測】ラベルの数値は、自プロジェクトのトラフィックで
   再計測してから確定値として扱う
...
- `reasoningEffort` は正規化済み。既定値の一次情報が矛盾しているため（上記）、**未指定なら wire に
  載せない**実装を崩さないこと（`drift-landmines.md`「世代ゲートされた能力ノブ」）

## 確認ソース

OpenAI: `platform.openai.com/docs/guides/realtime-vad`（eagerness の意味・auto=medium・server_vad の
挙動）、公式 OpenAPI spec `github.com/openai/openai-openapi`（v2.3.0。`default:` 注記 — eagerness
auto・8/4/2秒・create_response/interrupt_response true・threshold 0.5 等、noise_reduction・
idle_timeout_ms・`RealtimeReasoningEffort`）、`.../guides/realtime-models-prompting`（effort 運用・
...
`.../docs/live-api/capabilities`（手動 VAD ≥500ms）、`.../docs/live-guide`。OpenAI 分は 2026-07-12、
Gemini 分は 2026-07-11 時点のスナップショット。`stale_days` 超過後は再検証必須。
