...
完了を主張しないターン（進捗報告・状況説明・CI 待ち）は差分が変わっていても評価者を呼ばない
（v0.5.1）。`eval_base` は進めないので、見送った差分は次に完了を主張したターンでまとめて評価
される。差し戻し中（前回 BLOCK）だけは文面に関係なく評価する（「状況を説明します」で抜けられない）。
「完了」「完成」は述語の形（完了しました / すべて完了 / 行末の完了）だけを拾い、「CI 完了を待って
います」「完了通知」のような名詞用法は拾わない。根拠は 2026-09-18 の実測: 57 評価のうち 16 回が
完了を主張していないターンで、評価者は「完了は主張していない」としか言えなかった。
