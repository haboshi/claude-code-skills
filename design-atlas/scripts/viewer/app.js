...
function screenNode(s,x,y,w,withPorts){
 ...
 const figure=src?`<button class="shot-open" data-preview="${s.id}" aria-label="${esc(s.name)}のスクリーンショットを拡大"><img src="${src}" alt="${esc(s.name)}の画面" draggable="false"></button>`:'<span class="historical-shot">画像なし</span>';
...
function renderCatalog(){
 ...
 $('#catalog').innerHTML=filtered.map(n=>`<button data-select="${n.id}" title="${esc(n.name)}"><i></i>${esc(n.name)}<small>${n.code||n.number||''}</small></button>`).join('');
}
...
function tag(status){return `<span class="status-tag ${status}">${statusText[status]}</span>`}
...
function relationList(e){return D.relations.filter(r=>r.a===e.id||r.b===e.id).map(r=>{const other=r.a===e.id?r.b:r.a;return `<div class="relation-item"><button data-jump="${other}" data-mode="er">${esc(label(other))} ↗</button><small>${esc(label(r.a))} ${r.ca} → ${r.cb} ${esc(label(r.b))}<br>${esc(r.label)}${r.proposed?' · 設計候補':''}</small></div>`}).join('')}
function sourceButton(id,find){const s=source(id);if(!s)return '';
 const text=s.kind==='note'?`${s.note??id}${s.range?' '+s.range:''}`:`${s.path??id}${s.range?' '+s.range:''}`;
...
function preview(id,key){const s=screen(id);if(!s||!s.images.length)return;viewingScreen=s;
 ...
 const stamp=shown.captured_at?shown.captured_at.slice(0,10):'撮影日時の記録なし';
 const caption=`${shown.historical?'現行版より古い参考画像':'SCREEN CAPTURE'} / ${stamp}`;
 openViewer(s.name,caption,s.images.map(i=>`<button data-image="${esc(i.key)}" class="${i.key===imageKey?'active':''}">${esc(i.label)}</button>`).join(''),imageSrc(shown.key)?`<img src="${imageSrc(shown.key)}" alt="${esc(s.name)}：${esc(shown.label)}">`:'<p>この画像は同梱されていません。</p>');
}
