...
依頼文と直前の会話から次の 4 項目を自分で決める。4 項目が会話から一意に定まるか既定で足りるなら、
`AskUserQuestion` を使わずそのまま検索を始める。

| 項目 | 選択肢 | 既定 | 会話からの読み取り方 |
|------|--------|------|------|
| **調査タイプ** | `quick` / `deep` | `quick` | 「深掘り」「比較して」「レポート」「まとめて」「複数の観点」→ deep。単発の事実確認・URL 指定 → quick |
| **検索カテゴリ** | `web` / `news` / `images` / `videos` | `web` | 「ニュース」「最新」「今週」→ news、「画像」「写真」→ images、「動画」→ videos |
| **言語** | `ja` / `en` / `auto` | `auto`（依頼文の言語に追従） | 英語圏の技術情報は en を併用してよい |
| **出力形式** | `text` / `file` | `text` | 「ファイルに」「保存して」「レポートを作って」→ file |

`AskUserQuestion` を使うのは、読み違えると調査の中身が別物になり、かつ会話から決められない 1 点だけ
（例: 「A 社について調べて」で、企業概要か直近のニュースかで検索が変わる）。質問は 1 問に絞り、推奨の
選択肢を先頭に置く。読み取りが弱い項目は既定で進め、結果の冒頭に採った前提を 1 行で書く。
...
各ソースの内容は自分の言葉（間接話法）で 1〜2 文に言い換える。原文のまま残すのは引用符付きの短い語句を
1 つまでとし、出典を添える。次の例と同じ形で書く:

<example>
<user>Riverton Ledger と Coast Dispatch が Harbor Bridge 閉鎖をどう報じたか調べて比較して</user>
<response>
[search.py "Harbor Bridge closure Riverton Ledger" -t news]
[search.py "Harbor Bridge closure Coast Dispatch" -t news]
両紙とも基本的な事実は一致している。橋は 3 月 3 日、溶接部の亀裂が見つかって閉鎖され、州は修理に約 8 か月を見込む。違うのは力点で、Ledger は地域経済への影響として扱い、Dispatch は資金配分の失敗として枠づけ、社説で閉鎖を「entirely foreseeable」と評した。両方読むと、Ledger は今誰が困っているかを、Dispatch はなぜこうなったかを説明しており、片方だけでは全体像にならない。
</response>
<rationale>正解の理由: 2 紙の一致点と相違点を軸に構成し、どちらの記事の要約にもなっていない。各紙の内容は 1〜2 文の間接話法で伝え、原文をそのまま使ったのは引用符付きの短い語句 1 つだけ。それでも具体的で完結している。</rationale>
</example>
