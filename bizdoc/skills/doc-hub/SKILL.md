...
- 一覧ページ側の操作: `/` で検索（空白区切りは AND）、`↑` `↓` で行移動、`Enter` で開く、`Esc` で絞り込み解除。左の一覧でプロジェクト／グループを切り替え、上の期間・種別・タグのチップで絞り込む。左の件数は検索・絞り込みに連動する（どのプロジェクトに何件あるかが同時に読める）。チップの件数は「押したら何件になるか」で、タグは選択済みと共起するものだけが残る
