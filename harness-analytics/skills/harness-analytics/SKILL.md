...
4. **レポート生成**
   ```bash
   node ${CLAUDE_PLUGIN_ROOT}/scripts/build-report.js
   ```
   `reports/latest.html` と `latest.md` を生成（LLM 分析があればマージ）。**ブラウザは自動で開かず**パスを提示する。
   PDF 化を求められたら `latest.md` を pdf-creator-jp に渡す。

5. **FetchDB 連携ドレイン**（config `sinks.fetchdb.enabled=true` かつ MCP `fetch-db` 接続時のみ・任意）
...
6. **報告**: 生成したレポートのパス、上位クラスター（件数・傾向）、注目すべき改善バックログ上位を簡潔に伝える。
   確信度は3段階（確認済み/高確度/未確認）で明示し、示唆であって自動適用しないことを添える。

## config（`~/.claude/harness-analytics/config.json`、無ければ既定）
